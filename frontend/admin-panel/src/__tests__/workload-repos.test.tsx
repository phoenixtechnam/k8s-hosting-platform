...
const MOCK_REPOS = [
  {
    ...
    url: 'https://github.com/org/production-workloads.git',
    branch: 'main',
  ...
  },
  {
    ...
    url: 'https://github.com/org/staging-workloads.git',
    branch: 'develop',
...
describe('WorkloadRepoSettings', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });

  it('renders the "Workload Repositories" heading', () => {
    mockApiFetch.mockReturnValue(new Promise(() => {}));
    render(<WorkloadRepoSettings />, { wrapper: createWrapper() });
    expect(screen.getByText('Workload Repositories')).toBeInTheDocument();
  });
  ...
  it('displays repositories from API response', async () => {
    ...
    expect(screen.getByText('staging-workloads')).toBeInTheDocument();
    expect(screen.getByText('https://github.com/org/production-workloads.git')).toBeInTheDocument();
  ...
  it('shows empty state when no repos are configured', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No workload repositories configured. Add one to get started.')).toBeInTheDocument();
    });
  });
