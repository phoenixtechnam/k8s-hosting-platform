...
import { useCatalogRepos, useSyncCatalogRepo } from '@/hooks/use-catalog-repos';
import { useDeployments, useCreateDeployment, useUpdateDeployment, useDeleteDeployment } from '@/hooks/use-deployments';
...
function DeployedWorkloadsTab() {
  ...
  const { data: response, isLoading, isError, error } = useDeployments(selectedClientId ?? undefined, 'runtime,database,service');
  const createDeployment = useCreateDeployment(selectedClientId ?? undefined);
  const updateDeployment = useUpdateDeployment(selectedClientId ?? undefined);
  const deleteDeployment = useDeleteDeployment(selectedClientId ?? undefined);
  const { data: catalogResponse } = useCatalog('runtime,database,service');

  const deployments = response?.data ?? [];
  const catalogEntries = catalogResponse?.data ?? [];
  const { sortedData: sortedDeployments, sortKey, sortDirection, onSort } = useSortable(deployments, 'name');
  ...
  const handleDeploy = async (e: FormEvent) => {
    e.preventDefault();
    if (!deployForm.name.trim() || !deployForm.catalog_entry_id) return;
    try {
      await createDeployment.mutateAsync({
        name: deployForm.name.trim(),
        catalog_entry_id: deployForm.catalog_entry_id,
      ...
      setDeployForm({ name: '', catalog_entry_id: '', replica_count: '1', cpu_request: '0.25', memory_request: '256Mi' });
      setShowDeploy(false);
    } catch { /* error via createDeployment.error */ }
  };
  ...
  const handleToggleStatus = (id: string, currentStatus: string) => {
    ...
    updateDeployment.mutate({ deploymentId: id, status: newStatus as 'running' | 'stopped' });
  };
      ...
      {showDeploy && selectedClientId && (
        <form onSubmit={handleDeploy} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4" data-testid="deploy-form">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-5">
            ...
            <div>
              <label htmlFor="wl-catalog-entry" className="block text-xs font-medium text-gray-700 dark:text-gray-300">Catalog Entry</label>
              <select id="wl-catalog-entry" className="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-700 dark:text-gray-100" value={deployForm.catalog_entry_id} onChange={(e) => setDeployForm({ ...deployForm, catalog_entry_id: e.target.value })} required data-testid="deploy-catalog-select">
                <option value="">Select catalog entry...</option>
                {catalogEntries.map((entry) => <option key={entry.id} value={entry.id}>{entry.name} ({entry.type})</option>)}
              </select>
            </div>
            <div>
            ...
            <div className="flex items-end">
              <button type="submit" disabled={createDeployment.isPending} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50" data-testid="submit-deploy">
                {createDeployment.isPending && <Loader2 size={14} className="animate-spin" />}
                Deploy
              </button>
            </div>
          </div>
          {createDeployment.error && (
            <div className="mt-3 flex items-center gap-2 text-sm text-red-600 dark:text-red-400" data-testid="deploy-error">
              <AlertCircle size={14} />
              {createDeployment.error instanceof Error ? createDeployment.error.message : 'Failed to deploy'}
            </div>
          )}
        </form>
      )}
            ...
            <table className="w-full" data-testid="deployments-table">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                  ...
                  <SortableHeader label="Catalog Entry" sortKey="catalogEntryId" currentKey={sortKey} direction={sortDirection} onSort={onSort} />
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {sortedDeployments.map((deployment) => (
                  <tr key={deployment.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-5 py-3.5 font-medium text-gray-900 dark:text-gray-100">{deployment.name}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400">{deployment.catalogEntryId ?? '—'}</td>
                    <td className="px-5 py-3.5">
                      <StatusBadge status={deployment.status as Parameters<typeof StatusBadge>[0]['status']} />
                    </td>
                    <td className="px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400">{deployment.replicaCount}</td>
                    <td className="hidden px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400 md:table-cell">{deployment.cpuRequest}</td>
                    <td className="hidden px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400 md:table-cell">{deployment.memoryRequest}</td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="inline-flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => handleToggleStatus(deployment.id, deployment.status)}
                          disabled={deployment.status === 'pending' || deployment.status === 'failed'}
                        ...
                        >
                          {deployment.status === 'running' ? <Square size={12} /> : <Play size={12} />}
                        </button>
                        {deleteConfirmId === deployment.id ? (
                          <>
                            <button type="button" onClick={() => handleDelete(deployment.id)} disabled={deleteDeployment.isPending} className="rounded-md bg-red-600 px-2.5 py-1.5 text-xs font-medium text-white hover:bg-red-700 disabled:opacity-50" data-testid={`confirm-delete-deployment-${deployment.id}`}>Confirm</button>
                          ...
                          <button type="button" onClick={() => setDeleteConfirmId(deployment.id)} className="rounded-md border border-red-200 dark:border-red-800 bg-white dark:bg-gray-800 px-2 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20" data-testid={`delete-deployment-${deployment.id}`}>
                            <Trash2 size={12} />
                          </button>
                        )}
                      </div>
                ...
                {deployments.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                      No deployments for this client.
                    </td>
                  </tr>
                )}
              </tbody>
...
function AvailableWorkloadsTab() {
  ...
  const { data: response, isLoading, isError, error } = useCatalog('runtime,database,service');
  const { data: reposResponse } = useCatalogRepos();
  const syncRepo = useSyncCatalogRepo();
  ...
  return (
    <div className="space-y-4" data-testid="available-tab">
      ...
      {isError && (
        <div className="flex items-center gap-2 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm text-red-700 dark:text-red-400" data-testid="error-message">
          <AlertCircle size={16} />
          <span>Failed to load catalog entries: {error?.message ?? 'Unknown error'}</span>
        </div>
      )}
