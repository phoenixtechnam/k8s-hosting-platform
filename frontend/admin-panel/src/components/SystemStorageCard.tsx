...
import { useSystemPvcStorage, useResizeSystemPvc } from '@/hooks/use-system-storage';

/**
 * System DB Storage card — mirrors MailStorageCard for the platform
 * CNPG cluster (system-db-1 / platform namespace).
 *
 * Three explicit reject paths surfaced from the backend with
 * operator-actionable error text:
 *   - SYSTEM_PVC_SHRINK_NOT_SUPPORTED
 *   - SYSTEM_PVC_SAME_SIZE
 *   - STORAGE_CLASS_NO_EXPANSION
 */
export default function SystemStorageCard() {
  const storage = useSystemPvcStorage();
  const resize = useResizeSystemPvc();
    ...
    return (
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-5">
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <Loader2 size={14} className="animate-spin" /> Loading system DB storage…
        </div>
      </div>
    );
  }
      ...
      <div className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 p-5">
        <div className="flex items-start gap-2.5">
          ...
          <div className="text-sm text-red-700 dark:text-red-300">
            Could not read system-db-1 PVC state.{' '}
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-5 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100" data-testid="system-storage-heading">
          System DB Storage
        </h2>
      </div>

      <p className="text-sm text-gray-600 dark:text-gray-400">
        Persistent volume backing the platform database
        (<code className="rounded bg-gray-100 dark:bg-gray-800 px-1">system-db-1</code> in{' '}
        <code className="rounded bg-gray-100 dark:bg-gray-800 px-1">platform</code> namespace).
        Affects every tenant — resize during a maintenance window if possible.
      </p>
      ...
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4 grid grid-cols-2 gap-3 text-sm">
        <KvRow label="Requested" value={`${currentGiB} GiB`} testId="system-storage-requested" />
        <KvRow label="Capacity (live)" value={`${capacityGiB} GiB`} testId="system-storage-capacity" />
        <KvRow
          label="Used"
          value={
            data.usedBytes != null
          ...
          testId="system-storage-used"
        />
        <KvRow
          label="Free"
          value={data.freeBytes != null ? formatBytes(data.freeBytes) : 'unknown'}
          testId="system-storage-free"
        />
        <KvRow label="StorageClass" value={data.storageClass} testId="system-storage-sc" />
        <KvRow
          label="Last resized"
          value={data.lastResizedAt ? new Date(data.lastResizedAt).toLocaleString() : 'never'}
          testId="system-storage-last-resized"
        />
      </div>
      ...
      {expandingFs ? (
        <div
          role="status"
          data-testid="system-storage-expanding"
      ...
      <div className="space-y-2">
        <label htmlFor="system-pvc-new-size-gib" className="block text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
          New size (GiB) — must be greater than current {currentGiB} GiB
        </label>
        <div className="flex items-center gap-3">
          <input
            id="system-pvc-new-size-gib"
            type="number"
            min={1}
            max={2048}
            step={1}
            ...
            data-testid="system-pvc-new-size-gib"
          ...
          <button
            ...
            disabled={!canSubmit}
            data-testid="system-pvc-resize-button"
        ...
        {wouldShrink ? (
          <p className="text-xs text-red-700 dark:text-red-300" data-testid="system-pvc-shrink-warning">
            Shrinking is NOT supported — would require snapshot+restore into a fresh smaller cluster.
          </p>
        ) : null}
        {wouldNoOp ? (
          <p className="text-xs text-amber-700 dark:text-amber-300" data-testid="system-pvc-no-op-warning">
      ...
      {postSuccess && !resize.isPending ? (
        <div
          role="status"
          data-testid="system-pvc-resize-success"
      ...
      {resize.isError ? (
        <div
          role="alert"
          data-testid="system-pvc-resize-error"
...
function ResizeConfirmModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      data-testid="system-pvc-resize-modal"
    >
      <div className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
              Resize system-db-1 storage to {newGiB} GiB?
            </h3>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Online expansion via Longhorn — platform-api and PG stay running throughout.
        ...
        <div className="flex justify-end gap-2">
          <button
            ...
            onClick={onClose}
            disabled={pending}
            data-testid="system-pvc-resize-cancel"
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={pending}
            data-testid="system-pvc-resize-confirm"
