          ...
          <tbody className="divide-y divide-gray-100">
            {records.map((r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                <td className="px-5 py-3 text-sm"><span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-medium">{r.recordType}</span></td>
                <td className="px-5 py-3 text-sm text-gray-900">{r.recordName ?? '@'}</td>
                <td className="px-5 py-3 text-sm font-mono text-gray-600 max-w-xs truncate">{r.recordValue}</td>
                <td className="px-5 py-3 text-sm text-gray-500">{r.ttl}</td>
                <td className="px-5 py-3 text-right">
                  {deleteConfirmId === r.id ? (
                    ...
                    <button type="button" onClick={() => setDeleteConfirmId(r.id)} className="inline-flex items-center gap-1 rounded-md border border-red-200 bg-white px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50" data-testid={`delete-dns-${r.id}`}>
...
function HostingTab({ clientId, domainId }: { readonly clientId: string; readonly domainId: string }) {
  ...
  return (
    <form onSubmit={handleSave} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-5" data-testid="hosting-settings-form">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="webroot" className="block text-sm font-medium text-gray-700">Webroot Path</label>
        ...
        <div>
          <label htmlFor="forward" className="block text-sm font-medium text-gray-700">Forward URL <span className="text-xs text-gray-400">(optional)</span></label>
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm" data-testid="protected-dirs-section">
      ...
      {!isLoading && !isError && (
        <div className="divide-y divide-gray-100">
          {dirs.map((dir) => (
            <div key={dir.id}>
              <div className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 cursor-pointer" onClick={() => setExpandedDirId(expandedDirId === dir.id ? null : dir.id)} data-testid={`dir-row-${dir.id}`}>
                ...
                <div className="flex items-center gap-2">
                  {deleteConfirmId === dir.id ? (
                    <div className="inline-flex items-center gap-1">
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(null); }} className="rounded-md border border-gray-200 bg-white px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50">Cancel</button>
                    </div>
                  ) : (
                    <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(dir.id); }} className="inline-flex items-center gap-1 rounded-md border border-red-200 bg-white px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50" data-testid={`delete-dir-${dir.id}`}>
                      <Trash2 size={12} /> Delete
                    </button>
                  )}
                </div>
  ...
  return (
    <div className="border-t border-gray-100 bg-gray-50 px-5 py-4" data-testid={`dir-users-${dirId}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5 text-sm font-medium text-gray-700"><Users size={14} /> Users</div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mb-3 flex items-end gap-2" data-testid={`create-user-form-${dirId}`}>
          <div>
            <label htmlFor={`un-${dirId}`} className="block text-xs font-medium text-gray-600">Username</label>
          ...
          <div>
            <label htmlFor={`pw-${dirId}`} className="block text-xs font-medium text-gray-600">Password</label>
          ...
          {users.map((u) => (
            <div key={u.id} className="flex items-center justify-between rounded-md bg-white px-3 py-2 text-sm">
              <div className="flex items-center gap-2">
                ...
                {!u.enabled && <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs text-red-600">disabled</span>}
              </div>
              <button type="button" onClick={() => deleteUser.mutate(u.id)} className="rounded-md border border-red-200 px-2 py-1 text-xs text-red-600 hover:bg-red-50" data-testid={`delete-user-${u.id}`}>
                <Trash2 size={12} />
              </button>
            </div>
          ))}
