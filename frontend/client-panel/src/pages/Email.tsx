  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900" data-testid="email-heading">Email</h1>
      </div>
      ...
      {!domainsLoading && emailDomains.length === 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-12 text-center shadow-sm" data-testid="email-not-enabled">
          <Mail size={48} className="mx-auto text-gray-300" />
          <h2 className="mt-4 text-lg font-semibold text-gray-900">Email Not Enabled</h2>
          ...
          <div className="flex gap-1 border-b border-gray-200">
            {[
              ...
              <button key={t.key} type="button" onClick={() => setTab(t.key)}
                className={clsx('border-b-2 px-4 py-2.5 text-sm font-medium', tab === t.key ? 'border-brand-500 text-brand-600' : 'border-transparent text-gray-500 hover:text-gray-700')}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{mailboxes.length} mailbox{mailboxes.length !== 1 ? 'es' : ''}</p>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4" data-testid="create-mailbox-form">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="mt-1 flex">
                <input className="flex-1 rounded-l-lg border border-r-0 border-gray-300 px-3 py-2 text-sm" value={form.local_part} onChange={e => setForm({ ...form, local_part: e.target.value })} required placeholder="john" data-testid="mailbox-local-part" />
                <span className="inline-flex items-center rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-sm text-gray-500">@{domainName}</span>
              </div>
            </div>
            <div>
              ...
              {mailboxes.map(mb => (
                <tr key={mb.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3.5">
                    <div className="font-medium text-gray-900">{mb.fullAddress}</div>
                    {mb.displayName && <div className="text-xs text-gray-500">{mb.displayName}</div>}
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={() => handleOpenWebmail(mb.id)} className="inline-flex items-center gap-1 rounded-md border border-brand-200 bg-white px-2.5 py-1.5 text-xs font-medium text-brand-600 hover:bg-brand-50" data-testid={`webmail-${mb.id}`}>
                      ...
                      {deleteConfirmId === mb.id ? (
                        <div className="flex gap-1">
                          ...
                          <button type="button" onClick={() => setDeleteConfirmId(null)} className="rounded-md border border-gray-200 px-2.5 py-1.5 text-xs font-medium text-gray-600">Cancel</button>
                        </div>
                      ) : (
                        <button type="button" onClick={() => setDeleteConfirmId(mb.id)} className="inline-flex items-center gap-1 rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50" data-testid={`delete-mailbox-${mb.id}`}>
...
function AliasesTab({ clientId, emailDomains }: { readonly clientId: string; readonly emailDomains: readonly { id: string; domainName: string }[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{aliases.length} alias{aliases.length !== 1 ? 'es' : ''}</p>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4" data-testid="create-alias-form">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-medium text-gray-700">Source</label>
              <div className="mt-1 flex">
                <input className="flex-1 rounded-l-lg border border-r-0 border-gray-300 px-3 py-2 text-sm" value={form.source} onChange={e => setForm({ ...form, source: e.target.value })} required placeholder="support" data-testid="alias-source" />
                <span className="inline-flex items-center rounded-r-lg border border-gray-300 bg-gray-50 px-3 text-sm text-gray-500">@{domainName}</span>
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">Forward to (comma-separated)</label>
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100">
          {aliases.map(a => (
            <div key={a.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50">
              <div className="flex items-center gap-2">
                <span className="font-medium text-gray-900 text-sm">{a.sourceAddress}</span>
                <ArrowRight size={14} className="text-gray-400" />
                <span className="text-sm text-gray-600">{a.destinationAddresses.join(', ')}</span>
              </div>
              <button type="button" onClick={() => deleteAlias.mutate(a.id)} className="inline-flex items-center gap-1 rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50" data-testid={`delete-alias-${a.id}`}>
                <Trash2 size={12} /> Delete
              </button>
            </div>
          ))}
