...
spec:
  ...
  duration: 2160h  # 90 days
  renewBefore: 360h
  commonName: "*.panel-ip.platform.local"
  dnsNames:
  - "*.panel-ip.platform.local"
  issuerRef:
...
1.2.3.4  admin.panel-ip.platform.local
1.2.3.4  client.panel-ip.platform.local
...
curl -I https://admin.panel-ip.platform.local/admin/
```
...
spec:
  ...
  tls:
  - hosts:
    - panel-internal.platform.local
    secretName: frontend-tls-internal
  rules:
  - host: panel-internal.platform.local
    http:
      paths:
      - path: /admin(/|$)(.*)
        pathType: ImplementationSpecific
...
```bash
# /etc/hosts
1.2.3.4 panel.platform.local
curl https://panel.platform.local/admin/
```
