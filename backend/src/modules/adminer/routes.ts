...
export async function adminerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/clients/:clientId/adminer/login', {
    ...
    // The auto-login URL is a relative path under /adminer/ on the client
    // panel. The client panel's nginx proxies /adminer/* to the dedicated
    // Adminer proxy server (port 8081) running inside the backend container.
    // This means Adminer is accessed on the same origin as the client panel
    // — no extra ports or CORS issues.
    const loginUrl = `/adminer/auto-login?token=${token}&clientId=${clientId}`;
