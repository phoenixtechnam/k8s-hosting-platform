...
export async function applicationUpgradeRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/admin/application-instances/:id/upgrade', {
    ...
    const record = createUpgradeRecord({
      ...
      toVersion,
      triggeredBy: ((request as unknown as Record<string, unknown>).user as { id?: string } | undefined)?.id ?? 'unknown',
      triggerType: 'manual',
    });
  ...
  app.post('/admin/application-upgrades/batch', {
    ...
    const userId = ((request as unknown as Record<string, unknown>).user as { id?: string } | undefined)?.id ?? 'unknown';
