...
const { startImagePressureWatcher } = await import('./image-pressure-watcher.js');
...
describe('image-pressure-watcher', () => {
  beforeEach(() => {
    ...
    // Default: classifyImage returns not-protected
    mockClassifyImage.mockReturnValue({ protected: false });
  ...
  it('skips protected images (system images in use)', async () => {
    ...
    mockClassifyImage.mockReturnValue({ protected: true });
