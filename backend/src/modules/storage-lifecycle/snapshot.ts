...
export async function snapshotTenantPVC(
  ...
  // LocalHostPathStore now ships a PVC volumeSpec (PSA allows PVCs whose
  // underlying PV is hostPath); call ensureJobMountResources first so
  // the cluster-scoped PV + tenant-namespace PVC exist before the Job's
  // Pod is admitted. S3Store has no ensureJobMountResources (emptyDir
  // needs no pre-staging), so guard the call.
  if (typeof opts.store.ensureJobMountResources === 'function') {
    await opts.store.ensureJobMountResources(k8s, opts.namespace, archivePath);
  }
