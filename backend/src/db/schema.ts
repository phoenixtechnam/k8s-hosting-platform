...
export const catalogEntries = mysqlTable('catalog_entries', {
  ...
  volumes: json('volumes').$type<Array<{ local_path: string; container_path: string; description?: string; optional?: boolean }> | null>(),
  resources: json('resources').$type<{ recommended: { cpu: string; memory: string; storage?: string }; minimum: { cpu: string; memory: string; storage?: string } } | null>(),
...
export const deployments = mysqlTable('deployments', {
  ...
  status: mysqlEnum('status', ['deploying', 'running', 'stopped', 'failed', 'deleting', 'upgrading', 'pending', 'deleted']).notNull().default('pending'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow().onUpdateNow(),
}, (table) => [
